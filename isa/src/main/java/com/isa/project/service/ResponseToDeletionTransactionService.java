...
public class ResponseToDeletionTransactionService {
    ...
    @Transactional
    public void responseToApproveDeletionTransactional(Long requestId, Long userId) {
            deletionService.remove(requestId);
            appUserService.remove(userId);
    }

    @Transactional
    public void responseToDeclineDeletionTransactional(ResponseToDeletionRequest response, Long requestId) {
            deletionService.remove(requestId);
